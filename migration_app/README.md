...
```bash
migration_app$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
migration_app$ sam logs -n HelloWorldFunction --stack-name migration_app --tail
```
...
```bash
migration_app$ pip install pytest pytest-mock --user
migration_app$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name migration_app
```
